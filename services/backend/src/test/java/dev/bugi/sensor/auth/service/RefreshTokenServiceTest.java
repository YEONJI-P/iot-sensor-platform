...
public class RefreshTokenServiceTest {
    ...
    void save_rotates_existing_token_in_place() {
        RefreshToken existing = new RefreshToken("EMP001", "old_token", now.plusSeconds(7 * 86400));
    ...
    void get_returns_stored_token() {
        RefreshToken stored = new RefreshToken("EMP001", "token123", now.plusSeconds(7 * 86400));
    ...
    void get_returns_null_when_expired() {
        RefreshToken stored = new RefreshToken("EMP001", "token123", now.minusSeconds(1));
        when(refreshTokenRepository.findById("EMP001")).thenReturn(Optional.of(stored));
    ...
    void validate_returns_false_when_token_mismatch() {
        RefreshToken stored = new RefreshToken("EMP001", "other_token", now.plusSeconds(7 * 86400));
        when(refreshTokenRepository.findById("EMP001")).thenReturn(Optional.of(stored));
