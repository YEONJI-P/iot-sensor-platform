...
수신 경로 밖 스케줄러:
- FreshnessScheduler: 기대 주기 초과 침묵 감지. 같은 구역 장치가 동시 침묵하면 사이트 사건(계획정지·게이트웨이 장애)으로 보고 WARNING 집계 1건, 이웃이 정상 수신 중인데 단독 침묵하면 개별 고장으로 CRITICAL + AX diagnose-freshness 원인진단(ax.enabled 시). 코호트/개별 각각 디바운스
...
## TODO: Device 텔레메트리 분리 (방법 1)

Phase 3에서 드러난 문제 — `Device.lastSeenAt`(수신 하트비트, 고빈도)가 설정 엔티티 Device에 얹혀 있어, `markSeen()`이 매 수신마다 행을 dirty시키고 `@LastModifiedDate`가 걸린 `Device.updatedAt`을 오염시킨다(수신 활발하면 `updatedAt ≈ lastSeenAt`). `updatedAt`은 원래 이름·타입·위치·임계값 같은 **설정 변경**만 감사해야 함.

- **정석(택함)**: `lastSeenAt`을 Device에서 떼어 별도 텔레메트리 테이블/엔티티(예: `device_status`/`device_heartbeat`)로 분리. 설정(Device)과 런타임 상태를 분리 → 감사 오염 해소 + 핫로우 회피(설정 행을 매 수신마다 안 씀).
- 손대는 곳: `SensorDataService.receive`(markSeen 경로), 스키마, `FreshnessScheduler`(getLastSeenAt 읽는 쪽), 관련 조회.
- 절충안(안 택함, 참고): 하트비트를 `@Modifying UPDATE device SET last_seen_at=? WHERE id=?`로 분리해 dirty-tracking 우회 — 범위 작지만 텔레메트리/설정 결합은 그대로.
- 현재 임시 상태: `Device.updatedAt`은 `@LastModifiedDate`라 오염된 채로 두고 이 작업에서 정리 예정.
