...
- explain(Python): `PORT`(기본 `23200`)
- frontend: `23000` (있을 경우)
- DB: `DB_*` (기본 로컬 PostgreSQL `5432/sensor_monitor`)
- **데모 compose(`docker-compose up`)는 각 프레임워크 관례 기본포트를 쓴다** — backend `8080`(`SERVER_PORT=8080`), explain `8000`(`PORT=8000`)으로 핀 고정. 평가자에게 익숙한 URL이 낫다. 자체 postgres는 호스트 `5433`으로 노출(공용 DB와 별개, 포폴용 독립 스택)

포트를 코드/설정에 하드코딩하지 말고 env로 주입한다(backend `server.port`, explain은 uvicorn `--port ${PORT}`). 로컬 기본값이 23100·23200대인 건 흔한 8080/8000과 부딪히지 않기 위해서고, 같은 머신에서 워크트리를 동시에 띄울 땐 오프셋(`23100 + INSTANCE`, 예: 워크트리1=`23101`)으로 격리한다.
...
- explain: `GET /health` (기본 `23200`, 데모 compose는 `8000`).
