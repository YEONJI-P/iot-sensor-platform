...
`Device`는 **설정**(이름·타입·위치·임계값·기대주기)만, 수신 하트비트 `lastSeenAt`은 **`DeviceStatus`**(테이블 `device_status`, `device_id` 공유 PK)가 갖는다. 되돌리지 말 것.

- 이유: `lastSeenAt`은 센서 수신마다 갱신되는 고빈도 값이라 Device에 두면 (1) 매 수신이 Device 행을 dirty시켜 `@LastModifiedDate`인 `Device.updatedAt`을 오염시키고(설정 변경 감사가 무의미해짐), (2) 설정 행이 매 수신마다 다시 쓰여 뜨거워진다.
- 수신 경로: `SensorDataService.markSeen()`이 `DeviceStatus`를 upsert(없으면 insert). Device는 건드리지 않는다.
